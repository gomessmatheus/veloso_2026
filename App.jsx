      ...
      {editing && editForm && (
        <Modal title={`Editar: ${brand.name}`} onClose={()=>setEditing(false)} width={500}
          ...
          <Field label="Janela de exclusividade (dias)">
            <Input type="number" min="0" value={editForm.exclusivityWindowDays||7} onChange={e=>setEditForm(f=>({...f,exclusivityWindowDays:Number(e.target.value)||7}))}/>
          </Field>
          <div style={{ fontSize:11, color:TX3, marginTop:4 }}>Usado no futuro para detectar conflitos de marca concorrente.</div>
        </Modal>
      )}
    </div>
  );
