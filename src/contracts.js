...
import { STAGES, PRODUCTION_RULES } from "../constants/tasks.js";
 ...
 *
 * For monthly contracts missing start/end dates, returns 0 (can't calculate)
 * but also emits a structured warning via the second return value.
 *
 * @returns {{ value: number, warning: string|null }}
 */
export function contractTotalWithWarning(c) {
  if (c.paymentType === "monthly") {
    if (!c.contractStart || !c.contractDeadline) {
      return {
        value: 0,
        warning: `"${c.company}" é mensal mas não tem datas de início/fim — valor não calculado.`,
      };
    }
  ...
  if (c.paymentType === "split") {
    ...
    if (inst.length) return { value: inst.reduce((s, i) => s + (Number(i.value) || 0), 0), warning: null };
  }
  return { value: c.contractValue || 0, warning: null };
}

/** Backwards-compat wrapper — returns just the number. */
export function contractTotal(c) {
  return contractTotalWithWarning(c).value;
}

/**
 * Collect data-quality warnings across all contracts.
 * Returns an array of human-readable warning strings.
 */
export function contractCalcWarnings(contracts) {
  const warnings = [];
  contracts.forEach(c => {
    const { warning } = contractTotalWithWarning(c);
...
/**
 * Convert a value to BRL.
 * Legacy version — returns the raw value when rate is missing (0 treated as 1×).
 * Kept for backwards compat in sum reductions.
 */
...
/**
 * Strict conversion — returns null when the exchange rate is 0 or missing.
 * Use this for display values where "—" is safer than a misleading 0.
 *
 * @returns {number|null}
 */
export function toBRLStrict(value, currency, rates) {
  if (currency === "BRL" || !currency) return value;
...
/**
 * Build commission entries for a contract.
 *
 * Etapa 4: respects per-contract `c.commissionRate` with fallback to COMM_RATE (0.20).
 */
export function getCommEntries(c) {
  if (!c.hasCommission) return [];
  /** Per-contract rate takes priority over global constant. */
  const rate = (typeof c.commissionRate === "number" && c.commissionRate > 0)
    ? c.commissionRate
    : COMM_RATE;
  ...
  if (c.paymentType === "split") {
    ...
    return insts.map((inst, i) => ({
      ...
      amount: Math.max(0, (Number(inst.value) || 0) - costPerInst) * rate,
  ...
  return [{
    key: "single", label: "Pagamento Único",
    amount: netTotal * rate,
    currency: c.currency, date: c.paymentDeadline, isPaid: !!paid["single"],
  }];
}
...
export function getNFEntries(c) {
  ...
  if (c.paymentType === "monthly") {
    ...
    while (cur <= e) {
      const key = `${cur.getFullYear()}-${String(cur.getMonth() + 1).padStart(2, "0")}`;
      entries.push({
        key, label: `NF ${MONTHS_SH[cur.getMonth()]} ${cur.getFullYear()}`,
        amount: c.monthlyValue || 0, currency: c.currency, isEmitted: !!nf[key],
      });
      cur.setMonth(cur.getMonth() + 1);
    }
    return entries;
  ...
  const total = contractTotal(c);
  return [{
    key: "single", label: "NF Única",
    amount: total, currency: c.currency, date: c.paymentDeadline, isEmitted: !!nf["single"],
  }];
}
